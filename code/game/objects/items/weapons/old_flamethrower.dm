...
//this one's sticking around for legacy purposes. that's it
/obj/item/legacy_flamethrower
	name = "ancient flamethrower"
	desc = "You are a veteran firestarter!"
...
/obj/item/legacy_flamethrower/Destroy()
	QDEL_NULL(weldtool)
	QDEL_NULL(igniter)
	QDEL_NULL(ptank)
	. = ..()

/obj/item/legacy_flamethrower/process(delta_time)
	if(!lit)
...
/obj/item/legacy_flamethrower/update_icon()
	cut_overlays()
...
/obj/item/legacy_flamethrower/afterattack(atom/target, mob/user, proximity)
	if(!proximity) return
...
/obj/item/legacy_flamethrower/attackby(obj/item/W as obj, mob/user as mob)
...
/obj/item/legacy_flamethrower/attack_self(mob/user)
	. = ..()
	if(.)
		return
	if(user.stat || user.restrained() || user.lying)	return
...
/obj/item/legacy_flamethrower/Topic(href,href_list[])
	if(href_list["close"])
		usr.unset_machine()
...
/obj/item/legacy_flamethrower/proc/flame_turf(turflist)
...
/obj/item/legacy_flamethrower/proc/ignite_turf(turf/target)
...
/obj/item/legacy_flamethrower/full/Initialize(mapload)
	. = ..()
